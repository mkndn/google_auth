...
final class CamUtils {
    ...
    static float linearized(int i) {
        float f = i / 255.0f;
        return f <= 0.04045f ? (f / 12.92f) * 100.0f : ((float) Math.pow((f + 0.055f) / 1.055f, 2.4000000953674316d)) * 100.0f;
    }
    ...
    public static float[] xyzFromInt(int i) {
        ...
        float f6 = fArr3[2];
        float[] fArr4 = fArr[2];
        return new float[]{(f * linearized) + (f2 * linearized2) + (f3 * linearized3), (f4 * linearized) + (f5 * linearized2) + (f6 * linearized3), (linearized * fArr4[0]) + (linearized2 * fArr4[1]) + (linearized3 * fArr4[2])};
    }

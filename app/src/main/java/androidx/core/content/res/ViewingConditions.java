...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        if (f10 >= 0.9d) {
            lerp = CamUtils.lerp(0.59f, 0.69f, ((-0.9f) + f10) * 10.0f);
        } else {
            lerp = CamUtils.lerp(0.525f, 0.59f, ((-0.8f) + f10) * 10.0f);
        }
        float f11 = lerp;
        ...
        double d2 = f;
        Double.isNaN(d2);
        float cbrt = (f13 * f) + (0.1f * f14 * f14 * ((float) Math.cbrt(d2 * 5.0d)));
        ...
        float f19 = (400.0f * pow2) / (pow2 + 27.13f);
        float[] fArr8 = {f16, f18, f19};
        return new ViewingConditions(yFromLStar, (f16 + f16 + f18 + (f19 * 0.05f)) * pow, pow, pow, f11, f10, fArr6, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt);
    }

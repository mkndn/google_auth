...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    private void buildShadowCorners() {
        ...
        paint.setShader(new RadialGradient(0.0f, 0.0f, f5 + f6, new int[]{i, i, this.mShadowEndColor}, new float[]{0.0f, f3 / (f4 + f3), 1.0f}, Shader.TileMode.CLAMP));
    ...
    public float getMinHeight() {
        ...
        float max = Math.max(f, this.mCornerRadius + this.mInsetShadow + ((f * 1.5f) / 2.0f));
        float f2 = (this.mRawMaxShadowSize * 1.5f) + this.mInsetShadow;
    ...
    public float getMinWidth() {
        float f = this.mRawMaxShadowSize;
        float max = Math.max(f, this.mCornerRadius + this.mInsetShadow + (f / 2.0f));
        float f2 = this.mRawMaxShadowSize + this.mInsetShadow;
        return max + max + f2 + f2;
    }

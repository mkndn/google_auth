...
public enum Status {
    ...
    public static final Status fromWireCode(String str) {
        Status[] values;
    ...
    public static boolean isRetryableError(Status status) {
        if (!NETWORK_ERROR.equals(status) && !SERVICE_UNAVAILABLE.equals(status) && !INTNERNAL_ERROR.equals(status) && !AUTH_SECURITY_ERROR.equals(status) && !ACCOUNT_NOT_PRESENT.equals(status)) {
            return false;
        }
        return true;
    }
    ...
    public static boolean isUserRecoverableError(Status status) {
        if (!BAD_AUTHENTICATION.equals(status) && !CAPTCHA.equals(status) && !NEED_PERMISSION.equals(status) && !NEED_REMOTE_CONSENT.equals(status) && !NEEDS_BROWSER.equals(status) && !USER_CANCEL.equals(status) && !DEVICE_MANAGEMENT_REQUIRED.equals(status) && !DM_INTERNAL_ERROR.equals(status) && !DM_SYNC_DISABLED.equals(status) && !DM_ADMIN_BLOCKED.equals(status) && !DM_ADMIN_PENDING_APPROVAL.equals(status) && !DM_STALE_SYNC_REQUIRED.equals(status) && !DM_DEACTIVATED.equals(status) && !DM_REQUIRED.equals(status) && !THIRD_PARTY_DEVICE_MANAGEMENT_REQUIRED.equals(status) && !DM_SCREENLOCK_REQUIRED.equals(status)) {
            return false;
        }
        return true;
    }

...
public final class PrimesTimerDaggerModule_ProvideCustomDurationMetricServiceFactory implements Factory {
    ...
    public static PrimesTimerDaggerModule_ProvideCustomDurationMetricServiceFactory create(Provider provider, Provider provider2, Provider provider3, Provider provider4) {
        return new PrimesTimerDaggerModule_ProvideCustomDurationMetricServiceFactory(provider, provider2, provider3, provider4);
    }
    ...
    public static CustomDurationMetricService provideCustomDurationMetricService(Optional optional, Optional optional2, Provider provider, Provider provider2) {
        return (CustomDurationMetricService) Preconditions.checkNotNullFromProvides(PrimesTimerDaggerModule.provideCustomDurationMetricService(optional, optional2, provider, provider2));
    }

    @Override // javax.inject.Provider
    public CustomDurationMetricService get() {
        return provideCustomDurationMetricService((Optional) this.userConfigProvider.get(), (Optional) this.traceMetricServiceProvider.get(), this.withTracingImplProvider, this.implProvider);
    }
}

    ...
    private static void reflectivePrintWithIndent(MessageLite messageLite, StringBuilder sb, int i) {
        ...
        for (String str : treeSet) {
            ...
            if (substring.endsWith("List") && !substring.endsWith("OrBuilderList") && !substring.equals("List")) {
                String str2 = substring.substring(0, 1).toLowerCase() + substring.substring(1, substring.length() - "List".length());
            ...
            if (substring.endsWith("Map") && !substring.equals("Map")) {
                String str3 = substring.substring(0, 1).toLowerCase() + substring.substring(1, substring.length() - "Map".length());
            ...
            if (((Method) hashMap2.get("set" + substring)) != null) {
                if (substring.endsWith("Bytes")) {
                    if (!hashMap.containsKey("get" + substring.substring(0, substring.length() - "Bytes".length()))) {
                    }
                }
        ...
        if (messageLite instanceof GeneratedMessageLite.ExtendableMessage) {
            ...
            while (it.hasNext()) {
                printField(sb, i, "[" + ((GeneratedMessageLite.ExtensionDescriptor) entry.getKey()).getNumber() + "]", ((Map.Entry) it.next()).getValue());
            }
        }
